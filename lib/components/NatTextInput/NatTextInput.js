...
export const NatTextInput = ({
  ...
  const renderIcon = (iconData) => {
    if (iconData.svg) {
      ...
      const iconColor = (iconData.svg.color) ? iconData.svg.color : '#0FF';
      return (
    ...
    if (iconData.content) {
      return (
        <IconPress onPress={iconPress}>
          <InputIcon type={type} state={state} source={iconData.content} />
        </IconPress>
      );
    }
    return (
      <IconPress onPress={iconPress}>
        <InputIcon type={type} state={state} source={iconData} />
      </IconPress>
    );
  };
  ...
  return (
    ...
    >
      ...
      <InputWrapper type={type} state={state} defaultSize={defaultSize(size, numberOfLines)}>
        <TextInput
